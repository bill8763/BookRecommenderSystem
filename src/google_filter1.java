...
public class google_filter1 {
	...
	public static void search_filter(int no) throws IOException {
		
		
		FileReader FileStream = new FileReader("D:/K-core/Search1/"+no + "_" + "Lucene_output1.txt");
		BufferedReader reader = new BufferedReader(FileStream);
		String line = "";
		FileReader FileStream1 = new FileReader("D:/K-core/POS_filter/"+no + "_" + "filter_output1.txt");
		...
		BufferedWriter bw;
		bw = new BufferedWriter(new FileWriter("D:/K-core/Number_of_term/"+no + "_" + "number_of_term.txt",
				false));
...
//			}
			//上下限
			double upper = 9999;  //google 8  wiki 4
			double lower = 1;  //google 5  wiki 1
//			System.out.println("K=" + k);
			
			//根據搜尋結果過濾字詞
			//wiki要考慮回傳值不能為0
			if (lower <= Long.valueOf(value) && Long.valueOf(value)!=0) {
			//google版
			//if (lower <= value.length()) {
				if (!key.contains("+")) {
					if (Long.valueOf(value) <= upper) {
...
/*		Collections.sort(filtered_line, new Comparator<String>() {
			...
			public int compare(String o1, String o2) {
				...
				- Double.parseDouble(o2.split(",")[1]);
				int b=(int) (a/1000);
				return b;
			}
		});
		Object[] objline = filtered_line.toArray();
		bw = new BufferedWriter(new FileWriter("TermRank/"+no + "_" + "termRank.txt", false));
		...
		bw = new BufferedWriter(new FileWriter("D:/K-core/Pairs/"+no + "_" + "pairs.txt", false));
