...
public class K_core {
	...
	public void K_core_cal(int user_id,int concept_id,int topic_id) throws FileNotFoundException {
		...
		this.no = no;
		br = new BufferedReader(new FileReader("D:/DataTemp/User_K-core/Rank/"+no + "_" + "Rank.txt"));//讀出經過排序的NGD
		br2 = new BufferedReader(new FileReader("D:/DataTemp/User_K-core/Stem/"+no + "_" + "stem.txt"));//讀出字詞
		try {
	...
	void getK_code_value() {
		...
		String query="";
		try {
			bw = new BufferedWriter(new FileWriter("D:/DataTemp/User_K-core/K_core/"+no + "_" + "k_core.txt"));//寫入k-core運算結果
			bw2 = new BufferedWriter(new FileWriter("D:/DataTemp/User_K-core/Main_word/"+no + "_" + "main_word.txt"));//寫入核心特徵
...
//		kcore.simMin = input.nextDouble(); 
		kcore.K_core_cal(user_id,concept_id,topic_id);//計算K-core值
