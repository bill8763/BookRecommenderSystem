...
public class K_core {
	...
	public void K_core_cal(int no) throws FileNotFoundException {
		this.no = no;
		br = new BufferedReader(new FileReader("D:/K-core/Rank/"+no + "_" + "Rank.txt"));//讀出經過排序的NGD
		br2 = new BufferedReader(new FileReader("D:/K-core/Stem/"+no + "_" + "stem.txt"));//讀出字詞
		try {
			double sum = 0;
	...
	void getK_code_value() {
		...
		String query="";
		try {
			bw = new BufferedWriter(new FileWriter("D:/K-core/K_core/"+no + "_" + "k_core.txt"));//寫入k-core運算結果
			bw2 = new BufferedWriter(new FileWriter("D:/K-core/Main_word/"+no + "_" + "main_word.txt"));//寫入核心特徵
