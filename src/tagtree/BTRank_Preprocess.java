...
public class BTRank_Preprocess {
	...
	public static void main(int no, int type, String dirPath)
			throws IOException {
		...
		BufferedWriter bw1 = new BufferedWriter(new FileWriter(new File(dirPath
				+ "TagTree\\" + no + "_File\\BTRankPreValueResult.txt")));
		String output1 = "";
		...
		for (int i = 0; i < A_Term.size(); i++) {
			...
			for (int j = 0; j < H_TermCP.get(first).size(); j++) {
				...
				if (H_TermNGD.get(first).contains(second)) {
					...
					result = Double.parseDouble(H_TermCPValue.get(first)
							.get(second).toString())
							/ Double.parseDouble(H_TermNGDValue.get(first)
									.get(second).toString());
		...
		// ��X
		BufferedWriter bw2 = new BufferedWriter(new FileWriter(new File(dirPath
				+ "TagTree\\" + no + "_File\\BTRankPreResult.txt")));
		String output = "";
		for (int i = 0; i < A_Term.size(); i++) {
			output = A_Term.get(i);
	...
	public static void printHashtable(Hashtable<String, ArrayList> h,
			ArrayList<String> a) {
	...
	/*
	 * public static void main(String args[]) throws IOException { for(int
	 * i=1;i<=6;i++)main(i,2);
