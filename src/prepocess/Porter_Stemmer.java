...
public class Porter_Stemmer {
	public static void Stemmer(int no) throws IOException{
		...
		while ((str = BufferedStream.readLine()) != null) {
			   list.add(str);
			  }
		for(int i = 0; i < list.size() ; i++){
			//String term = list.get(i).split(",")[0];
		    // 利用正則表達式切割字符串
		    String[] term = list.toString().split("\\pP|\\pZ"); //分隔符號|
			for(int j = 0 ; j < term.length ; j++){
				term[j].toString();
				System.out.println("test" + term[j]);
				String t = term[j].toString();
				stemmer.stemming(t);
