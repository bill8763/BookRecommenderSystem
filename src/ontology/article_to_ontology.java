...
public class article_to_ontology  extends DBconnect{
	...
	public void article_to_database() throws IOException, SQLException{
		...
		concept_list.add((concept_num+1)+"%"+temp); //最後一個concept
...
//-------------------------資料寫入部份-------------------------------
		...
		String concept_id = concept_list.get(1).split("%")[0]; //暫時先跑 concept
		String[] concept_article = (concept_list.get(1).split("%")[1]).split("#"); //暫時先跑 concept
